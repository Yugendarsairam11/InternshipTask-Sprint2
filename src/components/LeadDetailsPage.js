...
import {leadUpdate} from '../actions/user.leadDetails'
import {useHistory} from 'react-router-dom'

function LeadDetailsPage(){
    console.log("the data initially is ",useSelector((state) => state.userData.isLeadAdded))
    const isLeadAdded=useSelector((state) => state.userData.isLeadAdded)
    ...
    const coursevalidation=(event)=>{
        ...
        if(value.length>20){
            courseref.current.innerHTML="length should not be more than 20 characters"
...
///-->Additonal Stuff
    ...
    <div className="leaddetailsbox">
        ...
        <br />
            ...
            <br />
            <div class="input-field col s12">
            <input  id="image" type="text" name="image" onBlur={imageValidation} />
            <label class="active" for="image"><b>Image URL</b></label>
            <span ref={imageref}></span>
        </div>
            <br />
            <div class="input-field col s12">
            <input  id="emailId" type="text" name="email" onBlur={emailvalidation} />
            ...
            <br/>
            <div class="input-field col s12">
            <input  id="emailId" type="text" name="number" onChange={numbervalidation} />
            <label class="active" for="username"><b>Mobile Number</b></label>
            <span ref={numberref}></span>
        </div>
            <br/>
            <div class="input-field col s12">
            <input  id="emailId" type="text" name="course" placeholder="UI/UX" onChange={coursevalidation} />
            <label class="active" for="username"><b>Course</b></label>
            <span ref={courseref}></span>
        </div>
            <br/>
            <div class="input-field col s12">
            <input  id="emailId" type="text" name="ads" placeholder="Google Ads/ Microsoft Ads" onChange={adsvalidation} />
            <label class="active" for="username"><b>Ads</b></label>
            <span ref={adsref}></span>
        </div>
            <br/>
            <div class="input-field col s12">
            <input  id="emailId" type="text" name="purpose" placeholder="Demo/Enroll" onChange={purposevalidation} />
            <label class="active" for="username"><b>Purpose</b></label>
            <span ref={purposeref}></span>
        </div>
            <br/>
        <div class="input-field col s12">
        <button  onClick={registeruser} class="waves-effect waves-light btn-large red">Add Lead</button>
        </div>
    </div>
</div>
    )
